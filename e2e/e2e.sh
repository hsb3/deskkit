#!/usr/bin/env bash
# e2e.sh — deskkit end-to-end SYSTEM-BEHAVIOUR suite (issue-scope: 1.0.0 cohesion lane).
#
...
#
# Usage:  bash e2e/e2e.sh          (from anywhere; it locates the repo from its path)
#         DESKKIT_BIN=/path/to/deskkit bash e2e/e2e.sh   (reuse a prebuilt binary)
#
...
echo "deskkit E2E system-behaviour suite"
echo "repo:        $E2E_REPO"
...
if [ -n "${DESKKIT_BIN:-}" ] && [ -x "${DESKKIT_BIN:-}" ]; then
  ...
  if make -C "$E2E_REPO" -n spa >/dev/null 2>&1; then
    ( cd "$E2E_REPO" && make spa )
    check "build the embedded SPA (Makefile spa -> internal/core/spa/dist)" $?
  else
    note "Makefile has no 'spa' target yet — building without it (placeholder page expected)"
  fi
  ( cd "$E2E_REPO" && go build -o "$E2E_BIN" ./cmd/deskkit )
  check "build deskkit from ./cmd/deskkit" $?
fi
export E2E_BIN
