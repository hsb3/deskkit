...
- **The deskkit MCP tool surface needs `deskkit` ON PATH and a fresh session.** The PM tools are
  served by `deskkit mcp-serve` (launched by the deskkit plugin's `.mcp.json`), so they exist
