...
- Retire `dismissed` from the `state` enum (forward migration; data-first down-remap per the
  0010/0012 migration precedent). `disposition` is the one human-judgment axis.
...
- Dropping enum values is a shipped-collection change — forward migration only, with the
  enum-extension down-remap pattern (0012 precedent: remap rows first, then shrink the enum).
