...
|---|---|---|
| R1 | mechanical | `is_entity_doc` (dir_kind ∈ {decisions,tasks,analyses,journal} and path ends `.md`) AND a key in `UNIVERSAL_FM_KEYS = [type, created, updated, tags, synopsis]` is missing from parsed frontmatter. |
...
| R4 | mechanical (FLAG-ONLY) | `dir_kind == decisions` and `.md`; `status ∉ DECISION_STATUSES = {proposed, accepted, rejected, superseded}`. No fixer. |
...
| **F-IGN** | `_structure/decisions/9999-ignore-fixture.md` | `---`↵`type: decision`↵`status: accepted`↵`---` then a body line. Missing `created`/`updated`/`tags`/`synopsis` ⇒ a genuine R1 finding; `status: accepted` is valid ⇒ no R4; `type: decision` under `_structure/decisions` ⇒ correctly placed, no R3. | R1 (but on an ignored path). | **yes** — matched by the `_structure/decisions/` prefix entry. |
