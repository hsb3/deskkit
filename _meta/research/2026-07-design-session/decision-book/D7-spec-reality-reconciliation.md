...
going forward (the spec's own prose, the standing `docs/development/specs/tool-surface.md`, or a generated/
...
(`docs/development/specs/pocket-librarian-v1-spec.md:1786-1787`). Shipped reality is narrower on the second
...
librarian tools at all, no gate (`docs/development/specs/tool-surface.md` §3). The first clause — a bare Claude
...
repo now has (`docs/development/specs/tool-surface.md`, shipped by #94) sits relative to the spec text going
forward.

## 2. Why now

#94 shipped `docs/development/specs/tool-surface.md` as an empirically-derived, counted map of all three
...
tool, a librarian tool rename) has nothing forcing the spec or `docs/development/specs/tool-surface.md` to move
...
- `surface-matrix.md § 0. Anchor and how this dossier extends it` — "Plugin TS MCP: reading
  ...
  source-level re-check of `docs/development/specs/tool-surface.md`'s TS count, no discrepancy found.
...
- `agent-symmetry.md § Verdict table — the 6 "accidental / unruled" asymmetries` — row 5,
  ...
  'default set … four' in §7.2). #94 shipped a NEW authoritative doc (`docs/development/specs/tool-surface.md`)
  rather than editing the spec, so the spec residue persists." Citation given there: spec
  `docs/development/specs/pocket-librarian-v1-spec.md:1784-1790` (route), `:399` + TOC `:14` ("six-tool"), `:1789`
...
- `agent-symmetry.md § New since the analysis (PR 112 — bears on the symmetry decision)` —
  "`docs/development/specs/tool-surface.md` is now the authoritative surface inventory (#94) ... It documents the
  ...
  (`docs/development/specs/tool-surface.md:1-152`)
...
- `docs/development/specs/pocket-librarian-v1-spec.md` TOC line 14: "5. [The six tools](#5-the-six-tools)."
- `docs/development/specs/pocket-librarian-v1-spec.md:399` — subcommand table: "`mcp-serve` | Expose the
  **six-tool core** as an MCP stdio server (model-facing; §7.2) | via gated `apply_fix` only."
- `docs/development/specs/pocket-librarian-v1-spec.md:1783-1799` (§7.2, "Outbound MCP server") — the full paragraph:
  ...
  `propose_fix`, `query`" — 4, not the 5 that `docs/development/specs/tool-surface.md` and `specs.go` now show,
  missing `record_feedback`).
- `docs/development/specs/tool-surface.md` §3 "Plugin TypeScript MCP server" (lines 94-107) and the Summary table
...
everywhere it appears (TOC, §3.3 table, §5 heading); point at `docs/development/specs/tool-surface.md` for the
...
`docs/development/specs/tool-surface.md` should gain a drift guard against a still-undetermined final tool set.
...
radius now; but `docs/development/specs/tool-surface.md` and the spec continue to disagree with no pointer between
...
- **Generated-artifacts precedent** (CLAUDE.md "Generated artifacts — never hand-edit" +
  ...
  keeps `docs/development/specs/tool-surface.md` or spec counts hand-maintained should say explicitly why this
...
- **Option A:** `docs/development/specs/pocket-librarian-v1-spec.md` — TOC line 14, §3.3 table line 399, §5
...
- **Option B:** `plugin/core/tools.ts`, `plugin/core/*` (new tool definitions + proxy
  ...
  unclaimed-tool problem at larger scale); `docs/development/specs/tool-surface.md` §3 + Summary table counts
  change from 4 to 4+N.
- **Option C:** no spec text change yet; `docs/development/specs/tool-surface.md` gains a `status`/authority
...
- **Drift-guard feasibility for prose.** Option A's "point at `docs/development/specs/tool-surface.md` instead of
...
- **No re-run of `docs/development/specs/tool-surface.md`'s empirical probe.** Per `surface-matrix.md § 8`, the
  counts here were source-verified, not re-run against a live build; re-run the JSON-RPC probe
  in `docs/development/specs/tool-surface.md`'s "How the counts were derived" section for a fresh empirical count.
