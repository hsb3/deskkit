...
| **3. Record** | Each ruling → ADR in `docs/decisions/` (append-only, cited where it binds); spec deltas drafted (`docs/pocket-librarian-v1-spec.md`, `docs/pm-system-v1-spec.md`, `schema/`); charter updated if direction moves | ADRs merged; specs no longer contradict rulings |
...
| C7 | **Spec/reality residue** | #94 shipped `docs/tool-surface.md` with verified counts | Spec still promises librarian tools over the TS `plugin/mcp` boundary that ships only 4 profile tools; reconcile spec → reality |
...
| `surface-matrix.md` | Operations × surfaces matrix (MCP by gate, CLI, TS plugin tools, skills, personas, TUI, admin console, human file paths) — who can do what from where, and its gating | **done.** Counts match `docs/tool-surface.md` exactly. Six unclaimed-surface findings: ride-along on the PM mount AND on the eino loop (the librarian system prompt is stale even without PM); 3 of 4 TS plugin tools claimed by no skill; PM `import` and the admin console claimed by no persona at all |
