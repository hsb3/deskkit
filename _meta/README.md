...
coding agents and humans on the next release. `_meta/` is tracked by default the only ignored path is `secrets/`, which stays machine-local while the empty dir survives a clone.
