> **Tracking:** #TBD, rollup over the ADR 0010-0017 build slices (2026-07-20 design session). Every v1-track mechanism, contract, and guard the session ruled, landed against shipped schema v1.
...
- [ ] #TBD `pointer-grammar-spec` — ADR 0010: normative spec section for the shipped pointer grammar
- [ ] #TBD `typed-reference-contract` — ADR 0011: the reference primitive + validation guard in `schema/`
- [ ] #TBD `item-type-validation` — ADR 0012: `CreateItem` hard-rejects unknown `items.type`
- [ ] #TBD `findings-lifecycle-completion` — ADR 0013: dismissed retirement, disposition-aware counts, provenance, adoption-log shrink
- [ ] #TBD `agent-integration-contract` — ADR 0014: contract spec section, tool gating, prompt fix, surface audit
- [ ] #TBD `desk-persona-bundle` — ADR 0014: the composed librarian + PM Claude Code bundle (v1 proof surface per ADR 0009)
- [ ] #TBD `prompt-governance` — ADR 0015: git-is-truth semantics + prompt-copy drift guard
- [ ] #TBD `tool-surface-drift-guard` — ADR 0016: `docs/tool-surface.md` pinned by a guard
- [ ] #TBD `ts-proxy-design` — ADR 0016: the TS `plugin/mcp` proxy design item (precedes implementation)
- [ ] #TBD `document-identity-hygiene` — ADR 0017: frontmatter id, doctype rename, cap guard
