...
created: 2026-07-23
purpose: >
  Build plan for #197 (label gate:v2-final): amend docs/element-model-v2-draft.md so the
...
_Amend `docs/element-model-v2-draft.md` to state explicitly which axis its
...
ruling, restated as the draft's own SS14 Out-of-scope, `docs/element-model-v2-draft.md:456-459`)._
...
- **Contract impact:** none. The change surface is a single `docs/`-scoped prose file
  (`docs/element-model-v2-draft.md`). No API, DB, codegen, or other source-of-truth surface is
...
- SS6.1 names the software lifecycle: "Phase machine (unchanged): `draft -> in-review ->
  ...
  weekly-checkin / retro cadence." (`docs/element-model-v2-draft.md:198`)
- SS6.4 states the verification gate: "`test-plan` is only the *plan*; nothing recorded
  ...
  reads." (`docs/element-model-v2-draft.md:224-229`) The SS12 disposition table marks gap S8
  "Folded in" on this basis (`docs/element-model-v2-draft.md:418`).
- SS5.3 and SS11 establish a **precedent pattern** elsewhere in the same document: a `claim`'s
  ...
  orthogonal to any workflow `state`**" (`docs/element-model-v2-draft.md:148-150`), and SS11
  restates it as "the design precedent for the `claim` status axis... an **orthogonal judgment
  axis, not a lifecycle `state`**" (`docs/element-model-v2-draft.md:375-381`), modelled
...
item phase machine is `queue -> work -> review -> terminal` (`docs/pm-system-v1-spec.md:398`,
full machine `docs/pm-system-v1-spec.md:471-501`), explicitly "a small, code-enforced machine,
deliberately separate from `status_label`" (`docs/pm-system-v1-spec.md:473`) and "data-in-code;
extending it... is a code change, not a config change -- deliberate" (`docs/pm-system-v1-spec.md
:500-501`). Gates bind on `items.type` at specific edges via `desk_config` YAML rules
(`docs/pm-system-v1-spec.md:494-498`); the shipped default set carries exactly two rules -- `task
...
(`docs/element-model-v2-draft.md:357-387` mentions `doctypes` only in the ADR 0012 type-count
...
1. **The mechanism already ships.** `schema/doctypes.yaml:54-55`'s `status.spec` enum is
   ...
   own word "(unchanged)" (`docs/element-model-v2-draft.md:198`) reads naturally as "this
...
2. **The PM machine is stated as architecturally closed to this kind of extension.** It is
   ...
   change, not a config change -- deliberate" (`docs/pm-system-v1-spec.md:500-501`). Reading (b)
   ...
   changes at all (`docs/element-model-v2-draft.md:456-459`). Interpretation (b) is the more
...
3. **The document is internally consistent with itself on the adjacent case.** SS5.3/SS11 already
   ...
   PM `state`" for `claim` (`docs/element-model-v2-draft.md:148-150`, `:375-381`), modelled on
...
ambiguity (`docs/element-model-v2-draft.md:418`) — so silence should not be mistaken for an
existing ruling. See Open question 1 below.

## Deliverables

Two disjoint units, both edits to the same single file
(`docs/element-model-v2-draft.md`) — they serialize on that file (see Parallelism below) but are
independently reviewable slices.

**A -- Axis statement (the orthogonality note).**
State explicitly, at or near SS6.1 (beside the "(unchanged)" parenthetical,
`docs/element-model-v2-draft.md:198`) and cross-referenced from SS6.4, that
...
- *Acceptance:* grep `docs/element-model-v2-draft.md` for a sentence naming the axis choice
...
`docs/element-model-v2-draft.md:433-448`), `engineering-spec` is not yet a live PM `items.type`
(ADR 0012 extends the vocabulary only "when the two-track lands,"
`docs/element-model-v2-draft.md:368-373`), and the owner has not ruled the wholesale default gate
...
- *Acceptance:* grep `docs/element-model-v2-draft.md` for the named rule (item type + phase edge +
...
Change surface: **`docs/element-model-v2-draft.md` only** -- a single `docs/`-scoped prose file.
...
| Prompt-drift (`scripts/check-prompt-drift.mjs`) | No | Explicitly scoped to `librarian/templates/librarian-system-prompt.txt` vs `docs/pocket-librarian-v1-spec.md` (`scripts/check-prompt-drift.mjs:29`) -- a different spec file entirely; confirmed by reading the script, it has no `element-model-v2-draft.md` reference (a repo-wide grep for the path across `scripts/`, `plugin/`, `librarian/`, `schema/`, `.github/` returned zero hits) |
...
| ---- | -------------- | --- |
| A (axis statement) | Serializes with B | Both edit `docs/element-model-v2-draft.md`; a single-file change is not a parallel-writer scenario -- one pass, one PR |
...
**Safe landing order:** a single PR, single commit, editing `docs/element-model-v2-draft.md` only
...
3. **Timing: should this amendment be a standalone PR that only touches
   `docs/element-model-v2-draft.md`'s SS6.1/SS6.4, or should it be folded into a larger doc pass
