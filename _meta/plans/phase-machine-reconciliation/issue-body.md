...
`docs/development/specs/element-model-v2-draft.md` §6.1 states the software lifecycle as
`draft → in-review → approved → building → shipped` (`docs/development/specs/element-model-v2-draft.md:198`), and §6.4
says a `test-run` / `verification` element gates `building → shipped` via `verified-by`
(`docs/development/specs/element-model-v2-draft.md:224-229`, disposition row S8 at `:418`).
...
(`docs/development/specs/pm-system-v1-spec.md:398`, machine at `:471-501`; verified live in the #126 v1 half,
...
- **(a)** the spec **document's own `status` axis** (frontmatter), orthogonal to the PM item's
  `phase` — the reading the "(unchanged)" parenthetical at `docs/development/specs/element-model-v2-draft.md:198`
...
- **(b)** a **replacement PM phase vocabulary** — which would mean redefining the rigid,
  code-owned state machine (`docs/development/specs/pm-system-v1-spec.md:500-501`).
...
**orthogonal to any workflow `state`**" (`docs/development/specs/element-model-v2-draft.md:148-150`, `:375-381`),
...
A docs-only amendment to `docs/development/specs/element-model-v2-draft.md` (model-track; no storage, no code). Two
edits:
...
1. **Axis statement.** State explicitly which axis `draft → in-review → approved → building →
   ...
   `claim`-status precedent (`docs/development/specs/element-model-v2-draft.md:148-150`). Land it at §6.1 (beside the
...
1. `docs/development/specs/element-model-v2-draft.md` states explicitly, in prose, **which axis**
...
2. That statement carries an **orthogonality note mirroring §5.3/§11** — i.e. it names the
   ...
   relationship to workflow `state` (`docs/development/specs/element-model-v2-draft.md:148-150`).
...
4. The amendment does **not** contradict the shipped PM machine: it does not assert
   ...
   redefining the rigid machine is a code change (`docs/development/specs/pm-system-v1-spec.md:500-501`).
5. `docs/development/specs/element-model-v2-draft.md` frontmatter stays `status: draft` — the flip to final is an
   owner call, not part of this amendment (see Out of scope).

## Dependencies & gates

Change surface: **`docs/development/specs/element-model-v2-draft.md` only** (a `docs/`-scoped prose edit).
...
- **Prompt-drift** (`scripts/check-prompt-drift.mjs`) — **does not fire**: it guards only
  `librarian/templates/librarian-system-prompt.txt` against `docs/development/specs/pocket-librarian-v1-spec.md`
...
- **Shipping any v2 storage or code** — no v2 collection, migration, gate-engine change, or store
  ...
  (`docs/development/specs/element-model-v2-draft.md:456-459`). This is a model-track amendment only.
...
- **Building the software / research planes** — live collections, patrol classification, the
  ...
  (`docs/development/specs/element-model-v2-draft.md:433-448`).
