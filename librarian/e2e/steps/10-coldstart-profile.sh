...
# 10 — cold-start + profile.  Sourced by e2e.sh; helpers (check/skip/note/section/dk/mcp_ts)
# and E2E_* state come from lib.sh. A brand-new empty folder becomes a working desk via
# `deskkit init`, config then resolves it by walk-up, and the plugin core validates the
# scaffolded profile through the TS MCP surface — all with zero personalization.
...
[ -f "$E2E_DESK/_knowledge/profile.yaml" ]
check "cold-start: _knowledge/profile.yaml written" $?
...
# --- profile resolves by walk-up and validates against schema v1 (via the plugin core) -----
if have_bun; then
  VAL=$(mcp_ts '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"profile_validate","arguments":{"path":"'"$E2E_DESK"'/_knowledge/profile.yaml"}}}')
  ...
  [ "$VALID" = "true" ]
  check "profile: scaffolded profile validates against schema v1 (plugin MCP profile_validate)" $?
  note "profile_validate returned valid=$VALID"
else
  skip "profile: schema validation via plugin MCP" "bun not on PATH"
fi
