...
- **`profile.yaml` (or `.json`/`.md`) — the structured, substitutable config.** Copy the
  plugin's shipped `profile.example.yaml`, fill in this deployment's identifiers and
  preferences (handles, repos, board, machines), and the system substitutes them into prompts
  and templates in place of hardcoded literals. Holds identifiers and preferences only —
  **never credentials** (those stay in `_meta/secrets/` or env, referenced by name via
  `secrets_ref`).
