...
The single personalization root both product lanes read. Two kinds of content:

- **`profile.yaml` (or `.json`/`.md`) — the structured, substitutable config.** Copy the
  shipped `profile.example.yaml` in this directory to `profile.yaml`, fill in this
  deployment's identifiers and preferences (handles, repos, board, machines), and the system
  substitutes them into prompts and templates in place of hardcoded literals. The real
  profile is gitignored — only `profile.example.yaml` ships — so nothing distributed carries
  a deployment identity. Holds identifiers and preferences only — **never credentials** (those
  stay in `_meta/operations/` or env, referenced by name via `secrets_ref`).
